...
class SinglyLinkedList {
     ...
     */
    // Psuedocode:
    // define a function that takes ina linked list object and determines if it is empty
    // check if Object.head.data == undefined
    // ifi it is object is empty
    isEmpty() {
        if(this.head == null){
...
const linkedList = {
    // head to refer to 1st item
    // node is object
    // node has some data
    // node has key call next
    // every node has a data and a next which points to the next node until you get to the end in which next is null
    // cant go backwards in linked lists
    head: {
        data: 1,
        next: {
            data: 2,
...
while (runner.next != null) { //stops one before null 
